...
public partial class LsKrxFutures : ConnectionBase, IMarket, IMarketKrx
{
	...
	private async Task<ResponseResult<MarketExecution>> RequestMarketExecutionSsf(string symbol)
	{
		try
		{
			...
			result.ExtraData = new()
			{
				{ "HighLimit", response.T8402OutBlock.UplmtPrice },
				{ "LowLimit", response.T8402OutBlock.DnlmtPrice },
	...
	public async Task<ResponseResults<MarketExecution>> RequestMarketExecutionHistory(string symbol, string begin = "", string end = "", decimal baseVolume = 0)
	{
		try
		{
			...
			{
				TimeExecuted = execution.Chetime[..9].ToDateTimeM(),
	...
	#region request Price Pack using t8465/t8466
	public async Task<ResponseResult<QuotePack<T>>> RequestPricePack<T>(QuoteRequest request) where T : Quote
	{
		...
		try
		{
			List<T8466OutBlock1> list = [];
			var nextKey = string.Empty;
			var ctsDate = string.Empty;
			do
			{
				var response = await RequestContinuousAsync<T8466>(LsEndpoint.FuturesChart.ToDescription(), new
				{
					t8466InBlock = new T8466InBlock
					{
				...
				if (response is null || response.T8466OutBlock1.Count == 0) break;

				list.AddRange(response.T8466OutBlock1);
				nextKey = response.NextKey;
				ctsDate = response.T8466OutBlock.CtsDate;
	...
	public async Task<ResponseCore> SubscribeMarketExecution(string symbol, bool connecting = true, string subscriber = "")
	{
		...
		if (trCode == nameof(FC9)) await SubscribeAsync(subscriber, nameof(DC0), symbol, connecting); // 야간파생 추가
