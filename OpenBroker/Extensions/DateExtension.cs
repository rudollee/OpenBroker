...
public static class DateExtension
{
	...
	private static readonly Dictionary<MarketZone, string> _ianaMap = new()
	{
		{ MarketZone.Utc,        "UTC" },
		{ MarketZone.Chicago,    "America/Chicago" },
		{ MarketZone.NewYork,    "America/New_York" },
		{ MarketZone.London,     "Europe/London" },
		{ MarketZone.Frankfurt,  "Europe/Berlin" },
		{ MarketZone.Paris,      "Europe/Paris" },
		{ MarketZone.Tokyo,      "Asia/Tokyo" },
		{ MarketZone.Seoul,      "Asia/Seoul" },
		{ MarketZone.HongKong,   "Asia/Hong_Kong" },
		{ MarketZone.Singapore,  "Asia/Singapore" },
		{ MarketZone.Bangkok,    "Asia/Bangkok" }
	};

	private static string GetIanaId(this MarketZone zone) => _ianaMap.TryGetValue(zone, out var id) ? id : "UTC";
	...
	/// <returns></returns>
	public static DateTime ToMarketTime(this DateTime utcDateTime, MarketZone zone)
	{
		if (utcDateTime.Kind == DateTimeKind.Unspecified)
			utcDateTime = DateTime.SpecifyKind(utcDateTime, DateTimeKind.Utc);

		string ianaId = GetIanaId(zone);
	...
	/// <param name="zone"></param>
	/// <returns></returns>
	public static DateTime Now(this MarketZone zone) => DateTime.UtcNow.ToMarketTime(zone);

	/// <summary>
	/// Convert Timezone to Date
	/// </summary>
	/// <param name="zone"></param>
	/// <returns></returns>
	public static DateOnly Today(this MarketZone zone) => DateOnly.FromDateTime(DateTime.UtcNow.ToMarketTime(zone));
	...
	{
		if (datetimeTxt863.Length == 9) datetimeTxt863 = DateTime.Now.ToKrxTradingDay().ToDate8Txt() + datetimeTxt863;
